...
**中文说明:** [README.md](README.md) · **Preview:** `qodex-v0.2.0` · **Validated platform:** Windows only
...
![Qodex console preview](assets/qodex-console-v020.png)
...
Double-click `Qodex.bat` to open the Windows launcher, then click Start and Open Console. The local console and control interface default to ports `3100` and `3110`. Empty allow lists deny messages by default. The console lists account-available models and reasoning efforts. Saving applies to the next turn across all conversations without restarting, preserving the speed tier; busy conversations block switching. Reset affects only the selected conversation: pending replies are cancelled, history/persona/long-term memory remain, and pre-reset history is excluded from the new model context. Already sent QQ messages are not recalled.
