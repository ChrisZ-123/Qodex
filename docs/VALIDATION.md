...
验证日期：2026-09-24。本次测试均使用隔离数据，不向真实 QQ 群发送消息。

- `npm test` 通过：20/20 组核心回归、部署间记忆目录隔离和 Windows DPAPI/启停夹具。
- 模型选择：账户列表分页与隐藏模型过滤、无效模型/强度拒绝、忙碌任务拒绝切换、保存到本部署配置、保留速度档位、新接口 CSRF 拒绝。
- 会话重置：旧回复隔离、历史保留、新会话不注入重置前历史、上下文起点持久化、创建会话过程中重置的竞争处理。
- 浏览器使用虚构数据验证模型保存后状态更新，并确认重置事件已出现、此前历史和记忆仍保留；发布截图更新为 v0.2.0 界面。
- 保留 v0.1.0 的首次会话目录创建、部署内记忆目录和控制台登录表单回归。

Windows CI 结果见 [Actions](https://github.com/ChrisZ-123/Qodex/actions)。原生确认弹窗曾使截图工具超时，重开页面后已核验重置成功；未据此修改产品行为。

以下为首版历史验证记录，不代表本版重新进行了真实 Codex 账号检查。
