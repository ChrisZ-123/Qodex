...
export function normalizeStickerEntry(raw) {
  ...
  return {
    ...
    noteSource: entry.noteSource === 'model' ? 'model' : 'manual',
    learningError: String(entry.learningError ?? ''),
    tags,
...
export function applyStickerNote(entries, id, patch = {}) {
  ...
  const next = normalizeStickerEntry({
    ...
    noteSource: patch.note !== undefined ? (patch.noteSource || 'manual') : target.noteSource,
    learningError: patch.note !== undefined ? '' : target.learningError,
